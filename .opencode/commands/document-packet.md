---
description: "Generate a byte-exact W2PP protocol packet spec into docs/packets/MSG_<Name>.md. Usage: /document-packet <MSG_Attack | _MSG_Attack> [extra focus]"
---

# Document Packet — MSG_* Spec Generator

The first argument is the packet to document. Accept either form: `MSG_Attack` or `_MSG_Attack` — normalize it: the struct is `MSG_*`, the type constant is `_MSG_*`. The optional second argument is any specific concern to focus on (e.g. "focus on anti-cheat checks"); if not provided, omit the extra-focus line below. Run once per packet. The deliverable is written to `docs/packets/MSG_<Name>.md`.
